...
def render_viewpoints_frames(model, cfg,render_poses, HW, Ks, frame_ids,ndc, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        intrinsic_matrix = np.array([
            [4.17605014e+03, 0.00000000e+00, 9.60000000e+02],
            [0.00000000e+00, 4.17605014e+03, 5.40000000e+02],
            [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]
        ])
        rays_o, rays_d, viewdirs = dvgo.get_rays_of_a_view(
            H, W, K, c2w, ndc, inverse_y=True,#render_kwargs['inverse_y'],
        ...
        render_result_chunks = [
            ...
            for ro, rd, vd in zip(rays_o.split(4096*32, 0), rays_d.split(4096*32, 0), viewdirs.split(4096*32, 0))
        ]
        end_time = time.time()
...
def mmap_decode():
    ...
    while(True):
        ...
        print("model mask size:", mask.shape)
...
with open('soldier_200K.csv', 'r') as file:
...
if args.render_360 > 0:
    ...
    sK = np.array([[1158.03, 0.0, 957.537964], [0.0,1158.03,501.716217],[0.0,0.0,1.0]])
    

    print("pose parameters", s_pos, center, up, )
    print("sK", sK)
    ...
    camera_height = 1.5
    target_height = 1.5
    center = np.array([0,  1.5, 0.0])

    for cam_id in range(args.render_360):
        angle = 6*cam_id #36 * (cam_id % 10)  # Angle in degrees
        radius = 2+0.017*cam_id
        # Convert angle to radians
        angle_radians = np.radians(angle)

        # Calculate camera position
        camera_x = radius * np.cos(angle_radians)
        camera_z = radius * np.sin(angle_radians)
        pos = np.array([camera_x, camera_height, camera_z])

        # Calculate lookat vector
        lookat = center - pos
        lookat = lookat / np.linalg.norm(lookat)

        # Calculate x-axis and y-axis for camera orientation
        xaxis = np.cross(up, lookat)
        xaxis = xaxis / np.linalg.norm(xaxis)
        yaxis = np.cross(lookat, xaxis)
        yaxis = yaxis / np.linalg.norm(yaxis)
        ...
        head_pose = data_array[cam_id+9900]
        head_x = float(head_pose[2]) / 100.0
        head_y = float(head_pose[3]) / 100.0
        head_z = float(head_pose[4]) / 100.0
        head_yaw = float(head_pose[5])+180.0
        head_pitch = -float(head_pose[6])
        head_roll = 180

        print(head_pose[0], head_y, head_z, -head_x, head_yaw, -head_pitch, head_roll)
        # Define the camera position and orientation
        camera_pos = glm.vec3(head_y, head_z, -head_x)  # Camera position y, z, -x

        yaw = glm.radians(head_yaw)   # Yaw angle in radians
        pitch = glm.radians(-head_pitch) # Pitch angle in radians Invert Pitch
        roll = glm.radians(head_roll)  # Roll angle in radians

        # Compute the rotation matrix from the yaw, pitch, and roll angles
        rotation = glm.mat4(1.0)
        rotation = glm.rotate(rotation, roll, glm.vec3(0.0, 0.0, 1.0))
        rotation = glm.rotate(rotation, pitch, glm.vec3(1.0, 0.0, 0.0))
        rotation = glm.rotate(rotation, yaw, glm.vec3(0.0, 1.0, 0.0))

        # Compute the translation matrix from the camera position
        translation = glm.translate(glm.mat4(1.0), -camera_pos)

        # Combine the rotation and translation to form the view matrix
        view = rotation * translation

        print(glm.inverse(view))

        sTs.append(glm.inverse(view))
        sKs.append(sK)
        HWs.append(HW)
        #frame_ids.append(0)
        frame_ids.append(cam_id % 300)
    ...
    def model_callback(model, render_kwargs, frame_id):
        ...
        if frame_id != args.render_start_frame:
            ...
            print("mask calculation takes:", time.time()-start_time)
            print("model mask size:", mask.shape, model.density.shape, model_receive.shape)
            # Assuming `mask` is a torch.Tensor
            num_true_elements = (mask == True).sum().item()

            # Print the result
            print("Number of True elements in mask:", num_true_elements)
            xyz_min = torch.Tensor(model_kwargs['xyz_min'])
            xyz_max = torch.Tensor(model_kwargs['xyz_max'])

            #--------------------------------------------------------------------------------
            # Mask shape
            mask_shape = np.array([240, 240, 270])

            # Generate linspace for each dimension
            x = np.linspace(xyz_min.cpu()[0], xyz_max.cpu()[0], mask_shape[0])
            y = np.linspace(xyz_min.cpu()[1], xyz_max.cpu()[1], mask_shape[1])
            z = np.linspace(xyz_min.cpu()[2], xyz_max.cpu()[2], mask_shape[2])

            # Create a meshgrid
            xx, yy, zz = np.meshgrid(x, y, z, indexing='ij')

            # Combine the coordinates into a single array of shape [240, 240, 270, 3]
            xyz_values = np.stack([xx, yy, zz], axis=-1)

            print(xyz_values.shape)
            #--------------------------------------------------------------------------------
