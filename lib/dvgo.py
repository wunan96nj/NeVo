...
class DirectVoxGO(torch.nn.Module):
    ...
    def sample_ray(self, rays_o, rays_d, near, far, stepsize, is_train=False, **render_kwargs):
        ...
        #print("sample rays", time.time()-start)
        #print(ray_pts.shape)

        mask_inbbox = ~mask_outbbox
        indices = torch.nonzero(mask_inbbox).squeeze()
        #print("sample rays", time.time() - start)

        ray_pts = torch.index_select(ray_pts, 0, indices)
        #print("sample rays", time.time() - start)

        ray_id = torch.index_select(ray_id, 0, indices)
        #print("sample rays", time.time() - start)

        step_id = torch.index_select(step_id, 0, indices)
        #print("sample rays", time.time() - start)
        # mask_inbbox = ~mask_outbbox
        # print("sample rays", time.time()-start)
        ...
        # print("sample rays", time.time()-start)
        #print(ray_pts.shape)
        return ray_pts, ray_id, step_id, stepdist, t_min
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        ray_pts, ray_id, step_id, stepdist, t_min = self.sample_ray(
                rays_o=rays_o, rays_d=rays_d, is_train=global_step is not None, **render_kwargs)
        interval = render_kwargs['stepsize'] * self.voxel_size_ratio
        #print('sample ray', time.time()-start_time)
        ...
        #print('mask_cache', time.time()-start_time)
        density = self.grid_sampler(ray_pts, self.density)
        #print('grid_sampler', time.time()-start_time)
        alpha = self.activate_density(density, interval)
        #print('activate_density', time.time()-start_time)
        ...
        #print('Alphas2Weights', time.time()-start_time)
        #self.fast_color_thres = 0.026
        if self.fast_color_thres > 0:
        ...
        # unique_filtered_indices = torch.tensor([[1, 1, 1]], dtype=torch.long, device='cuda')

        # if weights.shape[0] > 0:
        #     # Assuming ray_pts and weights are already tensors on the GPU
        #     ray_pts_gpu = ray_pts
        #     weights_gpu = weights

        #     # Use PyTorch operations instead of NumPy
        #     save_ijk_list = torch.round((ray_pts_gpu * torch.tensor([114.7619, 114.7186, 114.7619], device='cuda') + torch.tensor([120.5000,  29.2532, 120.5000], device='cuda'))).long()
        #     mask = weights_gpu > 0.026
        #     #rgb = rgb[mask]
        #     #mask =
        #     # Apply the mask to save_ijk_list to find the indices
        #     filtered_indices = save_ijk_list[mask]

        #     # If you need unique indices
        #     unique_filtered_indices = torch.unique(filtered_indices, dim=0)

        #     # # Initialize a mask of size 240x240x270 with False (0)
        #     # mask = torch.zeros((240, 240, 270), dtype=torch.bool, device='cuda')

        #     # # Unzip the indices into separate tensors
        #     # i_indices, j_indices, k_indices = unique_filtered_indices.t()

        #     # # Use advanced indexing to set the corresponding values in the mask to True
        #     # mask[i_indices, j_indices, k_indices] = True
        #     # self.mask_cache.mask= mask
        ...
        #print('rgb net', time.time()-start_time)
        # Ray marching
        ...
        #print('rgb_marched', time.time()-start_time)
        ret_dict.update({
            ...
            'raw_rgb': rgb,
            'ray_id': ray_id,
            #'unique_filtered_indices': unique_filtered_indices
        })
